...
// (cjenkins) #include <libplayercore/playercommon.h>
// (cjenkins) #include <libplayercore/playerconfig.h>
// (cjenkins) #include <replace/replace.h>
#include <sys/poll.h>
...
create_create(const char* serial_port)
{
  ...
  r = (create_comm_t*)calloc(1,sizeof(create_comm_t));
  assert(r);
  r->fd = -1;
...
create_open(create_comm_t* r, unsigned char fullcontrol)
{
  ...
  if(create_get_sensors(r, 100000) < 0)
  {
...
create_set_speeds(create_comm_t* r, double tv, double rv)
{
  ...
  else if(fabs(tv/rv) < 0.24) // assume we want to turn in place when rv is large compared to tv
  {
	...
	tv_mm = (int16_t)rint(fabs(rv) * CREATE_TVEL_MAX_MM_S *  
						  ((1.0-(fabs(tv/rv)/0.24))*0.5+0.5)); // transition more smoothly
