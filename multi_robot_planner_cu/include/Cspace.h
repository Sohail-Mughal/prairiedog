...
class Cspace
{
  public:
    ...
    Cspace(const vector<float>& the_start, const vector<float>& the_goal, int dimensions); // constructor
    Cspace(const Cspace& C);     // copy constructor
    ~Cspace();                   // destructor 
    
    bool Populate(const vector<float>& the_start, const vector<float>& the_goal, int dimensions); // populates or re-populates the structure, returns true on success
    ...
    bool BuildTreeV2(clock_t start_t, double clock_to_plan, int& steps, float prob_at_goal, float move_max, float theta_max, float resolution, float angular_resolution);  // this builds or continues to build the search tree. steps is decremented for each attempt to add a new point to the tree. The tree grows untill either the current time is clock_to_plan seconds past start_t or steps reaches 0 (if steps starts as a negative, then steps is ignored). the search moves at goal with prob_at_goal, in jumps no larger than move_max, returns true when it finds a better path, this version uses pruning when possible, A*-like re-linking when possible, and is otherwise based on an R, note move_max no longer used
